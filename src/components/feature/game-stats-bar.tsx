...
export function GameStatsBar({
  ...
  return (
    <section
      aria-label="Your statistics on this browser"
    ...
    >
      ...
      {showScore && (
        <>
          <Stat label="Best" value={String(stats.bestScore)} />
          <Stat label="Average" value={averageLabel} />
        </>
      )}
      ...
      <button
        type="button"
        onClick={() => {
          if (window.confirm('Reset your saved stats for this game? This cannot be undone.')) {
            reset();
          }
        }}
